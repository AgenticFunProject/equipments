...
import type { ContainerUnit, EquipmentType, LocalUser, Reservation } from "./types.js";
...
class SqlitePersistence implements StorePersistence {
  ...
  save(snapshot: StoreSnapshot): void {
    ...
    try {
      upsertMeta.run();
      this.db.exec(
        "DELETE FROM reservation_containers; DELETE FROM reservations; DELETE FROM containers; DELETE FROM users; DELETE FROM equipment_types; DELETE FROM store_snapshots;"
      );
