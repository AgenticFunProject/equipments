...
browser clicks locally, or `npm run test:ui:slow` when you need a one-second
delay between browser actions. Fresh Linux environments need
`npm run test:ui:install` before the UI test can launch Chromium because it
installs the Playwright browser plus required host system libraries.
